...
from scanners.static_memory import scan_static_memory
...
async def scan_file(
    ...
    tasks.append(scan_static_disasm(file_path))
    labels.append("static_disasm")
    tasks.append(scan_static_embedded(file_path))
    ...
    tasks.append(scan_static_memory(file_path))
    labels.append("static_memory")
    ...
    # Busca arquivos similares no historico (fuzzy hashes)
    try:
        similar_files = await database.find_similar(
            ssdeep_hash=hashes.get("ssdeep"),
            tlsh_hash=hashes.get("tlsh"),
            sha256_self=hashes.get("sha256"),
            min_score=30,
            limit=10,
        )
